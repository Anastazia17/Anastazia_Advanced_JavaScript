...
var formValidation = function (e) {
    ...
    if (result == false) {
        document.getElementById('form-box__area__headline').innerText = "Ваши данные не приняты!"
    }
    if (result == true) {
...
var mailValidation = (mail) => {
    ...
    if (mail.value.match(regexp)) {
        ...
        spanErrorText[1].innerText = 'Адрес эл. почты может содежрать латинские буквы, а также @, . - _';
